...
/**
 * Hand the downloader a scratch COPY of the cookie jar, never the real one.
 *
 * yt-dlp does not just read the file passed to --cookies: it writes its whole
 * jar back afterwards. Pointed at the extension's export, every tweet download
 * rewrote Instagram's cookie file — replacing the header with "generated by
 * yt-dlp", switching it to CRLF, and folding in cookies from every other site
 * it had touched (x.com and github.com were both observed). Handing over a
 * copy means the canonical export is never mutated.
 *
 * Returns null when there is no cookie file, so a caller can simply omit the
 * --cookies argument rather than pointing the downloader at a missing path.
 */
function makeCookieCopy(cookiesPath) {
  if (!cookiesPath || !fs.existsSync(cookiesPath)) return null;
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ssp-cookies-'));
...
export async function downloadItem(item, { config, run = defaultRun }) {
  ...
  // The cookie copy lives outside itemDir on purpose: the readdirSync below
  // treats everything in itemDir as downloaded media, so a cookie file there
  // would be recorded as a media row and, worse, left on disk.
  const cookieCopy = makeCookieCopy(config.COOKIES_PATH);
  const cookieArgs = cookieCopy ? ['--cookies', cookieCopy.copy] : [];

  const args = downloader === 'yt-dlp'
    ? [...cookieArgs, '--no-warnings', '-o', outTemplate, '--', item.url]
    : [...cookieArgs, '-D', itemDir, '--', item.url];
  ...
  try {
    ...
    // The copy holds a live session token, so remove it on every path,
    // including a thrown runner.
    if (cookieCopy) fs.rmSync(cookieCopy.dir, { recursive: true, force: true });
  }
